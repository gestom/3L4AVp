...
set size 0.9,0.7
set title "Average pedestrian localisation error over time - first person" 
set key top right 
set ytics 0.1 
set xlabel "Time [s]"
...
plot [0:34] [0:1.5] 'sum.tmp' using ($0/30):3 with lines title 'Lidar estimate [m]' lw 2,\
'' using ($0/30):4 with lines title 'Radar estimate [m]' lw 2,\
'' using ($0/30):5 with lines title 'Deep filter [m]' lw 2,\
'' using ($0/30):6 with lines title 'Weighted filter [m]' lw 2,\
'' using ($0/30):7 with lines title 'Switching radar [m]' lw 2,\
