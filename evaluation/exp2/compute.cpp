...
int main(int argc,char* argv[])
{
	...
	while(inFile >> ret >> cx >> cy >> rx >>ry >> rc >> lx >> ly >> lc)
	{
		...
		radTY.push_back(ry);
		radC.push_back(rc);
		if (n > 0 && ly == lasTY[n-1]) lx = lasTX[n-1]; 
		lasTX.push_back(lx);
		lasTY.push_back(ly);
		lasC.push_back(lc);
		if (n > 0){
	...
	transformRot(camX,camY,lasTX,lasTY,&lasX,&lasY,2000,3600,3);
...
//		printf("NumLas %i %f %f\n",numLas,lasY[i],lastLasY);	
	...
	sfD = sfD/datSize;
	kfD = kfD/datSize;
	printf("Sum Las/Rad/KF/SF %.1f %.1f %.1f %.1f %i\n",100*lasD,100*radD,100*kfD,100*sfD,datSize);
