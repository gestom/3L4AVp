...
static const int FEATURE_SIZE = 166;
...
public:
  ...
  void extractCluster(pcl::PointCloud<pcl::PointXYZI>::Ptr pc, int type);
...
Object3dDetector::Object3dDetector() {
  ...
  private_nh.param<int>("init_positives", init_positives_, 10);
  private_nh.param<int>("init_negatives", init_negatives_, 10);
  private_nh.param<int>("round_positives", round_positives_, 20);
  private_nh.param<int>("round_negatives", round_negatives_, 20);
  private_nh.param<int>("max_trains", max_trains_, 4);
...
const int nested_regions_ = 1;
int zone_[nested_regions_] = {10}; // for more details, see our IROS'17 paper.
void Object3dDetector::extractCluster(pcl::PointCloud<pcl::PointXYZI>::Ptr pc, int type) {
  features_.clear();
	...
	if(train_round_+1 < max_trains_) {
	  ...
	  if(type == 1) {
	    ...
	    if(positive_ == max_positives_ && negative_ < max_negatives_) {
	      svm_problem_.y[svm_problem_.l++] = -1; // -1, the negative label
	      ++negative_;
	      std::cerr << "negative: " << negative_ << std::endl;
	    }
	  }
	}
	//}
...
void Object3dDetector::train() {
  ...
  std::cerr << "positive_ ---------> " << positive_ << std::endl;
  clock_t t = clock();
