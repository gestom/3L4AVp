...
PeopleTracker::PeopleTracker() : detect_seq(0), marker_seq(0) {
  ...
  private_node_handle.param("target_frame", target_frame, std::string("world"));
...
void PeopleTracker::trackingThread() {
  ...
  while(ros::ok()) {
	...
	pub_trajectory_acc.publish(trajectory_acc);
      ...
      //	std::cerr << "cid " << poses[i].position.z << std::endl;
      // }
      /////////////////////////////////////////////////////////////      
    }
    fps.sleep();
  ...
  for(int i = 0; i < previous_poses.size(); i++) {
	...
	if(log_trajectories) {
	  ...
	  for(int k = 0; k < trajectory.poses.size(); k++) {
	    //std::cerr << trajectory.poses[k].position.x << " " << trajectory.poses[k].position.y << " [" << trajectory.poses[k].position.z << "] ";
	  }
	  //std::cerr << std::endl;
	}
      }
